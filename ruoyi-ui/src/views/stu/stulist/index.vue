...
<script>
import { listStulist, getStulist, delStulist, addStulist, updateStulist } from "@/api/stu/stulist";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listStulist(this.queryParams).then(response => {
        this.stulistList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            ...
            addStulist(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('stu/stulist/export', {
        ...this.queryParams
      }, `stulist_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
